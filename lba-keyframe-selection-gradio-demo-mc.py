...
def main_worker(options):
    ...
    if 'mnist' in options['dataset']:
        ...
        image_transform = transforms.Compose([
            transforms.Lambda(lambda img: img.convert("RGB") if img.mode == "RGBA" else img),  # RGBA를 RGB로 변환
    ...
    def keyframe_select_inference(img_list: List[Image.Image], text: str) -> dict:
    ...
    def inference_wrapper(filepaths, text):
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            text_input = gr.Textbox(label="Enter a question or text")
        submit_button = gr.Button("Submit")
        with gr.Row():
            gallery_output = gr.Gallery(label="Selected Keyframes")
            score_output = gr.Dataframe(headers=["Index", "Score"], label="Scores")

        submit_button.click(inference_wrapper, inputs=[file_input, text_input], outputs=[gallery_output, score_output])
