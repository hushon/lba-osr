...
from coop_clip import clip
from coop_clip.simple_tokenizer import SimpleTokenizer as _Tokenizer
...
class VanillaCLIP(nn.Module):
    ...
    def create_classifier_weights(self):
        ...
        for cname in self.classnames:
            texts = [template.format(cname) for template in templates]
            texts = orig_clip.tokenize(texts).cuda()
...
class CoOp(TrainerX):
    ...
    def load_model(self, directory='output/imagenet/CoOp/rn50_ep50_16shots/nctx16_cscFalse_ctpend/seed1', epoch=None):
        if not directory:
